...
conductor_codis3<-here::here("Codis_Bogdan_C.xlsx")
...
writexl::write_xlsx(list(codis =Codis_Bogdan), "Codis_Bogdan.xlsx")


```
## ii.Els enviem a un investigador que hi posara , el seu pertinent "agr" amb el nom de "Codis_Bogdan.C.xlsx" .

```{r ii,include=T}

# ho  rebo del investigador!!!
#cataleg_corregit1<-readxl::read_excel(conductor_codis3,col_types = "text")
#Codis_Bogdan_C [ codi bogdan corregit!]


cataleg<-readxl::read_excel(conductor_codis,col_types = "text")%>%select(domini,cod,Descripcio,agr)

cataleg_c<-readxl::read_excel(conductor_codis3,col_types = "text")%>%
  mutate(domini="Corregit")%>%
    select(domini,cod,Descripcio,agr)

cataleg<-rbind(cataleg,cataleg_c)

writexl::write_xlsx(list(codis =cataleg), "cataleg1.xlsx")
writexl::write_xlsx(list(codis =cataleg), "Codis_Bogdan_C.xlsx")


```


## iii.Seleccionem del dt1, aquells codis que siguin nomes pel PROJECTE, i ho tornem a enviar al investigador perque hi posi els AGR1.
```{r iv,include=T}
...
cataleg_Projecte<-cataleg2 %>% left_join(cataleg,by="cod")
writexl::write_xlsx(list(codis =cataleg_Projecte), "cataleg_Projecte.xlsx")

```
## iv.Un cop rebut, ja tindrem el CATALEG_PROJECTE , preparat.

```{r v,include=T}

#un cop rebut ja tindre el PROJECTE!!!!
